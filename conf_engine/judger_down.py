...
class DownJudger(Judger):
    def __init__(self, name, warn_text, steps, is_continue, peak_percent):
...
if __name__ == '__main__':
    ...
    percent_datas = [ 1.0*origin_datas[i]/origin_datas[i-1]-1 if i>0 else 0 for i in range(len(origin_datas))]
    print "origin_datas:", origin_datas
    print "percent_datas:", percent_datas

    judger1 = DownJudger("downJudger", "warning", [0.01, 0.02, 0.01], False, None)
    assert judger1.judge(origin_datas, percent_datas)

    judger2 = DownJudger("downJudger", "warning", [0.01, 0.02, 0.01], True, None)
    assert not judger2.judge(origin_datas, percent_datas)
    
    judger3 = DownJudger("downJudger", "warning", [0.01, 0.02], True, 0.05)
